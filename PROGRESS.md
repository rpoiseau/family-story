...
**Image principale de l'accueil (2026-09-02) : implémenté, en attente de validation.** L'utilisateur a fourni `family.png` (illustration satirique de toute la famille, style « portrait de groupe » cohérent avec le ton du site) à la racine du dépôt, avec la demande explicite de l'intégrer dans l'espace prévu sur `/`. Déplacé vers `src/assets/images/family.png` (emplacement cible documenté dans `.claude/rules/architecture.md`). `HomeView.vue` : le `v-sheet` placeholder (« Image principale à venir ») est remplacé par un `v-img` affichant `family.png` (`cover`, mêmes hauteurs responsives 320px/200px déjà en place). Aucun autre texte, route ou comportement modifié. `nvm use stable && npm run build` réussi (type-check + build), seul l'avertissement de taille de bundle déjà documenté subsiste (le fichier `family.png` fait ~2,9 Mo, à surveiller si le poids de la page devient un sujet, non traité sans demande explicite).

**Croix de fermeture sur les fiches de l'arbre (2026-09-02) : implémenté, en attente de validation.** Signalé par l'utilisateur : impossible de fermer une fiche personne au clic dessus. Cause probable : le seul bouton « Fermer » était en bas de la fiche (`v-card-actions`), hors champ pour les portraits longs (citations, tags) sans scroller jusqu'en bas. Ajout d'un bouton icône `mdi-close` fixe en haut à droite (`.dialog-close`, `position: absolute`) sur les deux dialogs de `FamilleView.vue` (fiche portrait complète et mini-fiche), en plus du bouton « Fermer » existant conservé. Build vérifié, testé en navigateur (ouverture/fermeture des deux dialogs par la croix).

Repéré en testant ce correctif, non traité (hors périmètre, signalé seulement) : les citations de Michel dans `family-members.json` (ex. `"Assigneu'nsexaunenfantcestdemode"`, `"Uu13423"`, `"MNi deNi de"`) semblent corrompues/illisibles — probablement une erreur de saisie antérieure. Aucune modification apportée sans demande explicite.

**Restylage de l'arbre généalogique en canvas D3 (2026-09-02) : implémenté, en attente de validation utilisateur.** Deux demandes explicites successives : (1) « améliore significativement le style et la vue de l'arbre généalogique », (2) après un premier essai en CSS pur jugé insuffisant : « enlève les icônes mariés/en couple etc, l'arbre ne ressemble pas vraiment à un arbre généalogique, utilise d3js pour faire l'arbre ». Aucune donnée, filiation ou route modifiée — uniquement le rendu visuel de la page `/famille`.

- Nouvelle dépendance explicitement demandée par l'utilisateur : `d3-shape` (+ `@types/d3-shape` en devDependency), actée dans `DECISIONS.md`. Utilisée uniquement pour calculer les tracés SVG courbes (`d3.linkVertical`) ; Vue reste seul maître du DOM (pas de sélection/binding D3).
- Nouveau composant `FamilyTreeCanvas.vue` qui remplace entièrement l'ancienne grille CSS (`FamilyTreeGenerationRow.vue` et `FamilyTreeMobileList.vue`, supprimés, plus aucune référence) : chaque personne est positionnée en pixels (génération = ligne, colonne calculée par le même algorithme de propagation à point fixe qu'avant) dans un unique canvas SVG scrollable (horizontal + vertical), commun au desktop et au mobile (tailles de colonne/ligne et carte réduites sous le breakpoint `mobile` de Vuetify). Les branches parent → enfant sont de vraies courbes générées par `d3.linkVertical()`, qui convergent et se séparent comme les branches d'un arbre réel.
- Connecteur conjoint/compagnon : trait simple (plein pour « conjoint », pointillé pour « compagnon »), **sans icône ni légende** — les cœurs et la légende ajoutés dans la première itération ont été retirés sur demande explicite. Le badge couronne sur la racine (ajouté dans la première itération) a également été retiré (même demande, « etc »).
- Étiquettes « Génération N » conservées, désormais en axe vertical fixe à gauche du canvas.
- `FamilleView.vue` simplifié : toute la logique de mise en page (colonnes, positions, tracés) a été déplacée dans `FamilyTreeCanvas.vue` ; la vue ne garde que le panneau, les deux dialogs (fiche portrait / mini-fiche) et le fond dégradé déjà validé.
- Type `FamilyTreeBlock` (devenu inutile) supprimé de `src/types/family-tree.ts`.
- Aucun texte, prénom, lien de parenté ou contenu familial modifié ou inventé ; `isMinor` n'est pas utilisé visuellement.

`nvm use stable && npm run build` réussi (type-check + build), avertissement de taille de bundle inchangé (déjà documenté ; `d3-shape` ajoute moins de 1 kB gzippé grâce au tree-shaking). Vérification manuelle dans Chrome (largeur desktop, 1400px) : branches courbes, trait pointillé Agathe/Tom, étiquettes de génération, fiche portrait au clic (Michel) et mini-fiche au clic (Jeanne) tous conformes. Limitation déjà connue de cet environnement (voir lot 1) : le redimensionnement de fenêtre vers une largeur mobile n'est pas pris en compte par l'outil de test ; le rendu mobile réel (constantes de mise en page réduites, défilement tactile horizontal + vertical) reste à confirmer par l'utilisateur sur mobile réel ou en réduisant la fenêtre du navigateur.
...
Source « GitHub Actions » activée dans Settings > Pages du dépôt distant `rpoiseau/family-story` (validé explicitement par l'utilisateur, exécuté via `gh api -X POST repos/rpoiseau/family-story/pages -f build_type=workflow`). URL cible : `https://rpoiseau.github.io/family-story/`. Le site restera 404/vide tant qu'aucun push n'a eu lieu.

Reste à faire, hors capacité de Claude Code : `git push` — toujours strictement interdit à Claude Code. Le dépôt local est en avance de 14 commits sur `origin/main`. L'utilisateur doit pousser lui-même pour que le workflow `.github/workflows/deploy.yml` se déclenche et publie le site.
...
- Lot courant : restylage de l'arbre généalogique en canvas D3 — implémenté, build vérifié, contrôle visuel desktop effectué, en attente de validation utilisateur
- Dernière étape achevée : réécriture du rendu de l'arbre en canvas SVG unique avec branches courbes `d3-shape`, retrait des icônes de statut conjugal et du badge racine
